...
- **⚠ coverage 비율만 비교하면 안 됩니다.** `ref_bench` 기준일(이 토픽은 2024-07)보다 새
  ...
  125 → 28로 바뀌었습니다. 해석은 §3.4에 있습니다 — 인간 서베이와 나란히 놓아야 읽힙니다.
...
같은 모델(flash)로 DB만 바꿔 생성하고, 인간 서베이를 기준선으로 함께 놓았습니다.
...
matched가 38 → 18로 줄어든 것만 보면 정전 문헌을 놓친 것처럼 보이지만, **판정 가능한 인용
안에서의 정확도는 30.4% → 64.3%로 올라 인간 서베이(54.2%)를 넘습니다.** 옛 논문을 고르는
능력이 나빠진 게 아니라, 인용 슬롯 128개 중 100개를 최신 논문에 쓰고 남은 28개를 더 잘
고른 것입니다.

**벤치마크는 이 배분을 볼 수 없습니다.** 참고문헌이 2024-07 이전 인간 서베이에서 나왔기
때문입니다. 2026년에 쓰는 서베이가 최신 논문을 많이 인용하는 것이 옳다면, 이 지표로는
그 판단을 할 수 없습니다 — 지표의 한계이지 산출물의 결함이 아닙니다.
...
| 신 DB | `deepseek-v4-flash-0731` | `parasail/fp8` | **OFF** | 2,064,277 / 143,354 | **$0.329** |
