...
> **발표·요약용 정리는 [`docs/`](docs/README.md)** — 무엇을 어떻게 생성했는지,
> 불완전한 벤치마크로 무엇을 평가했는지, 산출물 상태가 어떤지를 문서 5개로 갈라 뒀습니다.
...
| survey-search | RAG | 〃 | 〃 | **OFF** | 1,760,561 / 142,561 | $0.286 |
| survey-search | 3DGS | 〃 | 〃 | **OFF** | 1,799,780 / 164,047 | $0.298 |
| ss(fresh 끔) | 3DGS | 〃 | 〃 | **OFF** | 1,703,889 / 156,406 | $0.282 |

단가: `v4-pro` fp8 $0.67/$1.34 per M, `v4-flash-0731` fp8 $0.140/$0.280 per M.
아래 세 줄은 정가 환산입니다(reasoning이 꺼져 있어 실청구와 거의 같습니다).
**flash는 토픽·DB·검색 레이어에 관계없이 편당 $0.28~0.38**로, 회귀 검사 3토픽 6편이 약 $2 —
파일럿 1편 값입니다. **생성 10편 총액은 약 $4.95**입니다.

> survey-search arm 은 `run_demo.py` 를 안 거치므로 토큰 요약이 `experiment_times.log` 에
> 안 남습니다. 위 세 줄은 실행 로그(`eval_out/survey_search_arm*.log`)의 마지막 누적
> 토큰으로 환산했습니다. 나머지와 같은 조건(reasoning OFF)이라 실청구와 거의 같습니다.
>
> **검색 레이어를 바꿔도 생성 비용은 거의 안 변합니다.** 비용의 90%가 입력 토큰이고,
> 그 양은 `rag_max_out`·`outline_reference_num` 이 정하지 검색 방식이 정하지 않기 때문입니다.
> survey-search 는 여기에 **facet 분해 LLM 호출**이 별도로 붙는데(호출당 $0.0006,
> 다른 OpenRouter 키) 편당 $0.03 이하라 총액에 영향이 없습니다.
