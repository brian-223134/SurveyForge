...
- **⚠ 신 DB의 0.643을 개선으로 읽으면 안 됩니다.** `ref_bench` 기준일(이 토픽은 2024-07)보다
  새 논문은 분자·분모 양쪽에서 빠지는데, 신 DB 인용 128편 중 **100편이 그 이후**라
  분모가 125 → 28로 무너졌습니다. 실제로 맞힌 정전 문헌은 **38 → 18편으로 절반 이하**입니다.
  비율은 올랐지만 벤치마크가 추적하는 문헌은 덜 인용했습니다 (§3.4).
