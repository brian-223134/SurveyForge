...
- 결과 해석·비용 분석은 [`RESULT.md`](RESULT.md)에 있다. 이 문서는 **입력 고정과 검증**에 집중한다.
...
**비용 $1.998** (OpenRouter `total_usage` 차이, 토큰 카운터 추정이 아님).
카운트된 토큰의 정가 환산은 $1.673이므로 차액 **$0.325 ≈ 242k reasoning tokens**가
어느 카운터에도 잡히지 않은 채 출력 단가로 과금됐다 — 가시 출력의 **1.9배**, 총액의 **16%**.
**입력이 비용의 90%**이므로, 비용을 줄이려면 출력이 아니라 `--rag_num` / `--outline_reference_num`을 건드려야 한다.
