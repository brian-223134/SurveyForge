...
핵심 근거가 됩니다(→ [03-results.md](03-results.md) §3).
...
편당 **약 $2.00** (pro 모델 파일럿 실측: 아웃라인 $0.51 + 본문 $1.49).
입력이 비용의 90% — 참고문헌 초록을 단계마다 다시 보내는 구조 때문입니다.

> flash arm 의 비용은 별도로 실측하지 않았습니다. 단가가 더 낮으므로 위 값은 **상한**으로
> 읽으면 됩니다.
...
(→ [04-artifacts.md](04-artifacts.md) §3).
