...
| 항목 | 상태 |
|---|---|
| 서베이 생성 | ✅ 3편 (파일럿 1 + flash 구 DB 1 + flash 신 DB 1 — 마지막은 **진행 중**) |
| 논문 DB 최신화 | ✅ 589,123 → **908,819편**, 2024-09-25 → **2026-08-04** |
| 컷오프 게이트 통제 | ✅ 하드코딩 3곳 파라미터화, 회귀 테스트 10개 |
| SurveyBench 회귀 검사 | 🔄 구 DB 0.304 측정 완료, 신 DB 대기 |
...
| **신 DB (08-06)** | `v4-flash-0731` | **최신화본** | *(진행 중)* | | | | | |
...
### A. 신 DB 서베이 완료 후 회귀 검사 마무리 — **지금 진행 중**

`eval_out/rerun.log`가 돌고 있고, 끝나면 자동으로 `eval_out/eval_summary.md`가 생깁니다.
나오면 [`README.md`](README.md) §2·§5의 "측정 중" 행 두 개를 채우세요.

판단 기준:
- **coverage가 0.304에서 크게 떨어지면 희석** — 후보가 58.9만 → 90.9만으로 늘면서
  정전 문헌이 밀려난 것입니다. `rag_max_out`을 조정하거나 컷오프를 되돌려 확인합니다.
- **최신 인용 비율이 0%에서 유의미하게 오르면 증분이 인용까지 도달**한 것입니다.
