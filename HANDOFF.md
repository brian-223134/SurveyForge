...
| SurveyBench 회귀 검사 | ✅ 완료 — **정전 문헌 적중 38 → 18편**, 최신 인용 0% → 78.1% |
...
회귀 검사(2026-08-06, `eval_out/eval_summary.md`) 결과:
...
| **정전 문헌 적중** | **38** | **18** |
| 분모(기준일 이전) | 125 | 28 |
| coverage 비율 | 0.304 | 0.643 |
| 평가에서 무시된 인용 | 5 | **100** |
| 최신 인용 비율 | 0.0% | **78.1%** |

**증분은 인용까지 확실히 도달했습니다** — 인용의 78%가 2024-09 이후, 최신 인용일 2026-07-23.

**다만 정전 문헌 적중이 38 → 18편으로 절반 이하가 됐습니다.** coverage 비율이 0.643으로
올라간 건 개선이 아니라 분모가 125 → 28로 무너진 결과입니다(벤치마크 기준일 2024-07 이후
논문은 분자·분모 양쪽에서 빠짐). 최신 논문이 정전 문헌 자리를 밀어낸 것으로,
예상했던 **희석이 실제로 일어났습니다.**

선택지:
1. **그대로 둔다** — 최신성이 목적이면 합리적인 트레이드오프. 다만 "SurveyForge 재현"으로서는
   원 논문과 다른 물건이 됩니다.
2. **TRE 시간창을 손본다** — `sort_by_citation_period`가 창별 쿼터를 인용 논문 수에 비례해
   주는데, 신규 창이 둘(2024-25, 2026) 늘면서 옛 창 몫이 줄었습니다. `period`를 늘리거나
   창별 가중치를 두는 방향.
3. **`rag_max_out`을 키운다** — 서브섹션당 후보를 60에서 늘리면 옛 논문이 살아남을 여지가 커집니다.
   비용이 비례해 오릅니다(입력 토큰이 90%).

어느 쪽이든 **1토픽 n=1 결과**라는 점을 감안해야 합니다. 결정 전에 토픽을 2~3개 더 돌려
재현되는 현상인지 확인하는 편이 낫습니다 (편당 $0.33).
