...
**최종 갱신**: 2026-08-06 (2차 — 토픽 3개 회귀 검사)
...
| 항목 | 상태 |
|---|---|
| 서베이 생성 | ✅ 7편 — 파일럿 1 + flash 3토픽 × 2 DB (3DGS 신 DB는 **진행 중**) |
| 논문 DB 최신화 | ✅ 589,123 → **908,819편**, 2024-09-25 → **2026-08-04** |
| 컷오프 게이트 통제 | ✅ 하드코딩 3곳 파라미터화, 회귀 테스트 10개 |
| SurveyBench 회귀 검사 | ✅ 2토픽 재현 — 정확도 **~30% → ~62%**로 두 배 (3DGS 대기) |
...
| 3D Gaussian Splatting | 배포본 | *(진행 중)* | | | |
| 3D Gaussian Splatting | 최신화본 | *(진행 중)* | | | |
...
### A. 3DGS 결과 확인 후 결론 확정 — **거의 다 됨**

`eval_out/eval_<토픽>.md`. 2토픽에서 **같은 방향으로 재현**됐습니다.

| 토픽 | | 총인용 | 판정가능 | matched | **정확도** | 최신 인용 |
|---|---|---:|---:|---:|---:|---:|
| **RAG** | 인간 | 190 | 190 | 103 | 54.2% | 0% |
| | 구 DB | 130 | 125 | 38 | 30.4% | 0.0% |
...
**재현됩니다.** 구 DB 정확도가 두 토픽에서 30.4% / 29.5%로 거의 같고, 신 DB는
64.3% / 61.1%로 역시 거의 같습니다 — **~30%에서 ~62%로 두 배**입니다.

인간 대비는 토픽마다 다릅니다. RAG는 신 DB가 인간(54.2%)을 넘었지만 Multi-Agent는
인간(76.2%)에 못 미칩니다. 즉 "인간을 넘는다"가 아니라 **"옛 논문을 고르는 정확도가
DB 최신화로 두 배가 된다"**가 확인된 주장입니다.

**matched 절대값은 두 토픽 다 줄었습니다**(38→18, 31→11). 분모가 125→28, 105→18로
무너지기 때문입니다. `coverage` 비율(0.304→0.643, 0.295→0.611)도 같은 이유로 오릅니다.
**세 숫자 중 어느 것도 단독으로 읽으면 안 됩니다.** `compare_runs.py`가 셋을 함께 냅니다.

3DGS가 끝나면 세 토픽을 [`README.md`](README.md) §3.4 표에 합치고 결론을 확정하세요.
